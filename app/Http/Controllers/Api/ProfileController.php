...
//			if (count($user_check)) {
				...
				ProfilePicture::where('user_id', $user_details->id)
                                        ->update(['src' => html_entity_decode($request->profile_image)]);
...
//		}
        ...
        public function sendResetMyPassword() {
              
                $details = [
                    'title' => 'Mail from ItSolutionStuff.com',
                    'body' => 'This is for testing email using smtp'
                ];
                \Mail::to('eng.iessa.mostafa@gmail.com')->send(new \App\Mail\sendingEmail($details));
                
               return response()->json([
//
					'status_message' => 'Reset linl sent ,  Please review your email',
...
//			if (count($user_check)) {
				...
				$wallet = Wallet::whereUserId(auth()->user()->id)->firstorFail();
