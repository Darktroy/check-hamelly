 ...
 */
	...
	public function earning_chart(Request $request) {
		...
		if ($validator->fails()) {
			...
			$user = User::where('id', $driver_details->id)->where('user_type', $request->user_type)->firstorFail();
...
//			if (count($user)) {
				...
				if($trip->payment_mode=="Stripe" || $trip->payment_mode=="Stripe & Wallet")
				{
					...
					{
						$customer_id = PaymentMethod::where('user_id', $user_details->id)->firstorFail()->stripe_customer_id;
...
//			}
	...
	public function add_wallet(Request $request) {
		...
		if ($validator->fails()) {
			...
			$wallet = Wallet::whereUserId($user_details->id)->firstorFail();
