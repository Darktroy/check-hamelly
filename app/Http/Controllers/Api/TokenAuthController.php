...
class TokenAuthController extends Controller {
    ...
    public function test() {
        /*
        echo now().'   -   ';
        echo \Carbon\Carbon::now()->subMinutes(15)->isoFormat('hh:mm:00');
        ...
        $schData =  ScheduleRide::with('request','driver')->where('schedule_date',date('Y-m-d', strtotime(now())))
                ->where('schedule_time',$timeBefore15minutes)->get()->toArray();
            foreach ($schData as $key => $value) {
            dd($value['driver']['device_type']);
            }*/
        
//        echo \Carbon\Carbon::now()->addMinutes(180);
//        echo date('Y-m-d', strtotime(now()));
//        date('Y-m-d', strtotime($request->schedule_date))
    }
    ...
    public function register(Request $request) {
        ...
        } else {//if ($request->user_type == 'Rider' || $request->user_type == 'rider') 
            $rules = array(
                'mobile_number' => 'required|regex:/^[0-9]+$/|min:6',
                'user_type' => 'required|in:Rider,Driver,rider,driver',
                'password' => 'required|min:6',
        ...
        }//else of if ($request->user_type == 'Rider' || $request->user_type == 'rider') 
        ...
        if (!$validator->fails()) {
            ...
            if (count($user)) {
                ...
                if ($request->user_type == 'Rider' || $request->user_type == 'rider') {
                    ...
                    $driver_address->save();
                }//end of else for if ($request->user_type == 'Rider' || $request->user_type == 'rider') 
    ...
    public function login(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if (Auth::attempt([$db_id => $user_id, 
                'password' => $request->password, 'user_type' => $request->user_type, 'country_code' => $request->country_code])) {
...
//        }
    ...
    public function forgotpassword(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $user_check = User::where('mobile_number', $request->mobile_number)
                    ->where('user_type', $request->user_type)->firstOrFail();
//            dd($user_check);
//
//            if (count($user_check)) {
                ...
                User::whereId($user_check->id)->update(['password' => $password, 'device_id' => $request->device_id, 
                    'device_type' => $request->device_type, 'currency_code' => $currency_code]);
