...
//        echo(\Carbon\Carbon::now()->addHour(3)->isoFormat('hh:mm'));
        echo(\Carbon\Carbon::now()->addHour(3)->isoFormat('hh'));
        $data = $this->request_helper->sendSchNotification();
    ...
    public function car_types() {
        /*
          ...
          'POINT(".$request->pickup_latitude.' '.$request->pickup_longitude
          .")'),ST_GeomFromText(coordinates))) as available "))
          ->having('available','1')->where('status','Active')->first();
          $location_id = $match_location->id;
         */
        $trucks = CarType::get()->toArray();
        return response()->json(['data' => $trucks, 'status_code' => '1',]);
    }
...
//			if ($pickup_country == $drop_country) {
                ...
                if (!$match_location) {
                    return response()->json(['status_message' => trans('messages.location_unavailable'), 'status_code' => '0',]);
                }
//                                dd($match_location->id);  1
...
//                        dd($location_id);
                $nearest_car = DriverLocation::select(DB::raw('*, ( 6371 * acos( cos( radians(' . $request->pickup_latitude .
                                                ...
                                                ') ) + sin( radians(' . $request->pickup_latitude . ') ) * sin( radians( latitude ) ) ) ) as distance'))
                    ...
                    if ($get_fare_estimation['status'] == "success") {
                        ...
                        if ($data) {
                            foreach ($data as $fare) {
                                $fare_details[$fare->vehicle_id] = array('id' => $fare->peak_fare[0]->id,
                                    'car_id' => $fare->vehicle_id, 'price' => $fare->peak_fare[0]->price,
                                    'type' => $fare->peak_fare[0]->type);
                            }
                        }
                        ...
                        foreach ($nearest_car as $key => $list_car) {
                            ...
                            if (count($location) > 0) {
                                ...
                                $distanceFare = $list_car[$i]->car_type->manage_fare->per_km;
                                if ($km > 800) {
                                    $distanceFare = $list_car[$i]->car_type->manage_fare->distrance_800;
                                } elseif ($km > 600) {
                                    $distanceFare = $list_car[$i]->car_type->manage_fare->distrance_600_800;
                                } elseif ($km > 400) {
                                    $distanceFare = $list_car[$i]->car_type->manage_fare->distrance_400_600;
                                } elseif ($km > 100) {
                                    $distanceFare = $list_car[$i]->car_type->manage_fare->distrance_100_400;
                                } elseif ($km > 30) {
                                    $distanceFare = $list_car[$i]->car_type->manage_fare->distrance_30_100;
                                }
//                                                            $base_fare = round($list_car[$i]->car_type->manage_fare->base_fare + ($list_car[$i]->car_type->manage_fare->per_km * $km));
                                $base_fare = round($list_car[$i]->car_type->manage_fare->base_fare + ($distanceFare * $km));
...
//                                        'peak_id' => $fare_estimation,
                ...
                $cars = CarType::with(['manage_fare' => function($q) use($location_id) {
                                ...
                                ->whereIn('id', $vehicles)->where('status', 'Active');
...
//                        "peak_price" => $fare_estimation,
    ...
    public function updateriderlocation(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $user_check = User::where('id', $user_details->id)->firstorFail();
...
//        else {
        ...
        if ($request->request_id) {
            ...
            $rider = RideRequest::where('id', $request->request_id)->first()->toArray();
            $fare_estemation = $this->calculateFare($rider);
            $data = ['rider_id' => $rider->user_id,
                ...
                'fare_estemation' => $fare_estemation,
                'peak_price' => $peak_price];
            ...
            User::whereId($rider_id)
                    ->update(['device_id' => $request->device_id,
                        'device_type' => $request->device_type]);
            $data = $request->all();
            $fare_estemation = $this->calculateFare($data);
...
//			}
    ...
    public function sos(Request $request) {
        ...
        if ($request->input('mobile_number_of_emergency_contact') != '') {
            $request->replace(array('mobile_number_of_emergency_contact' => preg_replace("/[^\w]+/", "",$request->input('mobile_number_of_emergency_contact')), 
                'action' => $request->input('action'),
                'name_of_emergency_contact' => $request->input('name_of_emergency_contact'),
                'country_code_of_emergency_contact' => $request->input('country_code_of_emergency_contact'),
                'id' => $request->input('id_of_emergency_contact')));
        }
        if ($request->action != "view") {
            $rules = array('mobile_number_of_emergency_contact' => 'required|numeric', 'action' => 'required');
            $validator = Validator::make($request->all(), $rules);
...
//		if (count($user)) {
        $mobile_number = preg_replace('/^\+?' . $request->country_code_of_emergency_contact . '|\|' .$request->country_code_of_emergency_contact . '|\D/', '', ($request->mobile_number_of_emergency_contact));
        ...
        if ($request->action == 'update') {
            if ($emer) {
                return response()->json(['status_message' => trans('messages.mobile_number_exist'), 'status_code' => '0', 'contact_count' => $count,
                            'Emergency_contact_details' => $contact_details]);
            }

            $emercency = new EmergencySos;
            $emercency->name = $request->name_of_emergency_contact;
            $emercency->country_code = $request->country_code_of_emergency_contact;
            $emercency->mobile_number = $mobile_number;
            $emercency->user_id = $user_details->id;
            $emercency->save();
            $count = EmergencySos::where('user_id', $user_details->id)->get()->count();
            $contact_details = EmergencySos::where('user_id', $user_details->id)->get();
            return response()->json(['status_message' => "Added Successfully", 'status_code' => '1',
                        'contact_count' => $count, 'Emergency_contact_details' => $contact_details]);
        } else if ($request->action == 'delete') {
            $del = EmergencySos::findorFail($request->id);
            if ($del == null) {
                return response()->json(['status_message' => "Not found given request", 'status_code' => '0', 'Emergency_contact_details' => $count, 'contact_details' => $contact_details]);
            }
            ...
            return response()->json(['status_message' => "Delete Successfully", 'status_code' => '1', 'contact_count' => $count,
                        'Emergency_contact_details' => $contact_details]);
        } else {
            return response()->json(['status_message' => trans('messages.success'), 'status_code' => '1', 
                'contact_count' => $count, 'Emergency_contact_details' => $contact_details]);
        }
//		} else {
//
...
//		}
    ...
    public function save_schedule_ride(Request $request) {
        ...
        if (@$request->schedule_id) {
            $rules = array('schedule_date' => 'required',
...
//******************************
                $additional_fare = "";
                $peak_price = 0;
                $fare_estemation = $this->request_helper->calculateFare($request->all());
                if (isset($request->peak_id) != '') {
                    $fare = PeakFareDetail::find($request->peak_id);
...
//******************************
                ...
                $request_table->save();
                $array_sch = $request->all();
                $array_sch['schedule_id'] = $request_table->id;
                $array_sch['driver_group_id'] = null;
                $array_sch['rider_id'] = $rider_id;
                $array_sch['additional_fare'] = $additional_fare;
                $array_sch['peak_price'] = $peak_price;
                $array_sch['fare_estemation'] = $fare_estemation;
                
                $this->request_helper->sch_trip_find_driver($array_sch);
            }
...
//		}
    ...
    public function calculateFare($param) {
        $get_fare_estimation = $this->request_helper->GetDrivingDistance($param['pickup_latitude'],
