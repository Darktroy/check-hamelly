...
use App\Events\TripWasScheduled;
use App;
...
class DriverController extends Controller {
	...
	public function updatelocation(Request $request) {
		...
		if ($validator->fails()) {
			...
			$user = User::where('id', $user_details->id)
                                ->where('user_type', $request->user_type)
                                ->firstOrFail();
//			if (count($user)) {
				$driver_location = DriverLocation::where('user_id', $user_details->id)->first();
//                                dd(isset($driver_location));
                                //# this may produce an error the phase of trip id set never debugged as we deliver mvp     
				...
				if ($request->trip_id) {
					...
					// }
					if ($user->device_type == 3 && isset($driver_location)) {
				...
				if (isset($driver_location) && $driver_location->status == 'Trip'  ) {
...
//				if (count($user)) {
					...
					$data = RideRequest::where('id', $request->request_id)->firstorFail();
                                      
					if ($data->schedule_id!= '') {

//					ScheduleRide::where('id', $data->schedule_id)->update(['status' => 'Completed']);
					ScheduleRide::where('id', $data->schedule_id)->update(['driver_id' => $user->id]);
					...
					$polyline = @$this->request_helper->GetPolyline($data->pickup_latitude, 
                                                $data->drop_latitude, $data->pickup_longitude, $data->drop_longitude);

					// Create Trip
                                        
                                        $fare_estemation = $this->request_helper->calculateFare($data);
					...
					$trip->request_id = $data->id;
                                        $trip->total_fare = $fare_estemation;
...
//					$trip->peak_fare = $data->peak_fare;
					...
					$push_data = array('accept_request' => array('trip_id' => $trip->id));
                                        ...
                                        // time of schedule, driver that will be notified
                                        $driver = User::where('id', $data->driver_id)
                                                ->where('user_type','Driver')->orWhere('user_type','driver')->firstorFail();
/*                                        dd( $schedule_ride['schedule_date'].' '. $schedule_ride['schedule_time']);
                                        dd(Carbon::parse(sprintf('%s %s', $schedule_ride['schedule_date'], $schedule_ride['schedule_time'])));
                                        $dumy=[]; $dumy[] = $schedule_ride;
                                        $dumy[] = $schedule_ride['schedule_date'].' '. $schedule_ride['schedule_time'];*/
                                        $next_date =Carbon::parse(sprintf('%s %s', $schedule_ride['schedule_date'], $schedule_ride['schedule_time']));
                                        $difference = $next_date->diffInSeconds(Carbon::now()->subMinutes(1));
                                        /*
//                                        $dumy[] = $next_date;$dumy[] = Carbon::now();$dumy[] = $difference;
//                                        dd($dumy);
                                        
                                        event(new TripWasScheduled($driver,$schedule_ride,$request->request_id))
                                                ->delay($schedule_ride['schedule_date'].' '. $schedule_ride['schedule_time'])
                                                        ->subMinutes(1);
                                        
//                                        dispatch((new secondSch())->delay());

                                        
                                        event(new TripWasScheduled($driver,$schedule_ride,$request->request_id,$difference));
                                        dispatch((new \App\Console\Commands\quaterhour())->delay(Carbon::now()->addMinutes(1)));
                                        
                                        event(new TripWasScheduled($driver,$schedule_ride,$request->request_id))
                                                ->delay(Carbon::parse(sprintf('%s %s', 
                                                        $schedule_ride['schedule_date'], 
                                                        $schedule_ride['schedule_time']))->subMinutes(10))
                                                        ;
...
/*				
	...
	public function cash_collected(Request $request) {
		...
		if ($validator->fails()) {
			...
			Trips::where('id', $request->trip_id)
                                ->update(['status' => 'Completed', 'paykey' => @$request->paykey, 'payment_status' => 'Completed']);

			$trip = Trips::where('id', $request->trip_id)->firstorFail();
			...
			$push_data = array('trip_payment' => array('status' => 'Cash Collected',
                            'trip_id' => $request->trip_id,'driver_thumb_image' => $driver_thumb_image));
			...
			if (isset($schedule_ride) && $schedule_ride->booking_type == 'Manual Booking') {
				...
				$text = $text.trans('messages.cash_collected_payment_detail',['base_fare'=>$trip->base_fare,
                                    'time_fare'=>$trip->time_fare,'distance_fare'=>$trip->distance_fare,'total_fare'=>$trip->total_fare]);
...
//		}
		...
		if ($request->type == 'default') {
			$payout = PayoutCredentials::where('id', $request->payout_id)->firstorFail();
