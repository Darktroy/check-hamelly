...
class UserController extends BaseController
{
    ...
    //iESSA refactory for login 
    public function login_company(Request $request)
    {
        $data = $request->all();
        $company = null; $guard = null;
        $company = Company::where('mobile_number', $data['emailormobile'])->orWhere('email', $data['emailormobile'])->firstorFail();
        ...
        if (is_numeric($data['emailormobile'])) {
            $guard = Auth::guard('company')->attempt(['mobile_number' => $data['emailormobile'], 'password' => $data['password']]);
        }else{
            $guard = Auth::guard('company')->attempt(['email' => $data['emailormobile'], 'password' =>  $data['password']]);
        }
        ...
        foreach ($data as $key => $credential) {
            if ($key == 'email_phone') {
                if (is_numeric($credential)) {//Login by Phone number
                    ...
                    if ($data->user_type == 'Company') {
                                ...
                                if ($company) {
                                    if ($company->status == "Inactive") {
//                                        dd('company->status == "Inactive"');
                                        Auth::guard('company')->logout();
                                        return ['status' => 'false', 'error' => trans('messages.user.disabled_company_account'),
                                            'success' => 'true'];
                                    }
                                    Session::put('login_type', 'mobile_number');
                                    
                                    return redirect('company/dashboard');
//                                    return ['status' => 'true', 'error' => '', 'success' => 'false', 
//                                        'user_detail' => '+' . $company->country_code . ' ' . $company->mobile_number];
                                } else {
                                    return ['status' => 'false', 
                                        'error' => trans('messages.user.no_recognize') . SITE_NAME, 
                                        'success' => 'false'];
                                }
...
//                                }
    ...
    public function driver_register(Request $request)
    {
        ...
        } elseif ($request->step == 'car_details') {
            $rules = array(
                'vehicle_name' => 'required',
                'vehicle_number' => 'required',
