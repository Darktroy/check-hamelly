...
class Trips extends Model
{
    ...
    public $convert_fields = ['time_fare', 'distance_fare', 'base_fare',
        'total_fare', 'access_fee', 'driver_payout', 'owe_amount', 
        'remaining_owe_amount', 'applied_owe_amount', 'wallet_amount',
        'promo_amount','payable_driver_payout','cash_collectable',
        'commission','company_admin_commission','total_trip_fare','total_invoice','total_payout_frontend','cash_collect_frontend','driver_front_payout','rider_paid_amount','subtotal_fare','peak_amount','schedule_fare','driver_peak_amount','company_commission','driver_service_fee','driver_or_company_commission','company_earning'];
    /**
    ...
    protected $appends = ['vehicle_name','driver_name','rider_name',
        'rider_profile_picture','driver_thumb_image','rider_thumb_image',
        'date','pickup_time','drop_time','pickup_date_time','trip_time',
        'begin_date','payout_status','date_time_trip','driver_joined_at',
        'payable_driver_payout','cash_collectable','commission',
        'company_admin_commission','total_trip_fare','total_invoice',
        'total_payout_frontend','cash_collect_frontend','driver_front_payout',
        'rider_paid_amount','map_image','currency_symbol','status'];
  ...
  // Join with profile_picture table
        ...
        return $query->with(['payment'])
            ...
            ->where(function($query)  {
                if(LOGIN_USER_TYPE=='company') {
                    ...
                    $query->whereHas('driver',function($q1){
                        $q1->where('company_id',1)->orWhere('company_id',0);
                    });
                }
            })
    ...
    {
         return $total_fare = $this->attributes['base_fare'] + $this->attributes['time_fare'] + $this->attributes['distance_fare'] 
                 + $this->attributes['schedule_fare'] + $this->attributes['access_fee'] + $this->attributes['peak_amount'];
    }
    ...
    {
      return User::find($this->attributes['driver_id'])->first_name.' '.User::find($this->attributes['driver_id'])->last_name;
    }
    // get Rider name
