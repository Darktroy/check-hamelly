...
DB::unprepared($sql);
            ...
            if (Schema::hasTable('site_settings') && DB::table('site_settings')->count()) {
                $site_settings = DB::table('site_settings')->get();
                View::share('logo_url', url('images/logos/' . $site_settings[3]->value) . '?v=' . Str::random(4));
                define('LOGO_URL', 'images/logos/' . $site_settings[3]->value);
                define('PAGE_LOGO_URL', 'images/logos/' . $site_settings[4]->value);
                View::share('favicon', url('images/logos/' . $site_settings[5]->value) . '?v=' . Str::random(5));
            ...
            if (Schema::hasTable('country') && DB::table('country')->count()) {
                $country = DB::table('country')->get();
                View::share('country', $country);
            }
            if (Schema::hasTable('car_type') && DB::table('car_type')->count()) {
                $car_type = CarType::where('status', '=', 'Active')->get();
                View::share('car_type', $car_type);
            }

            if (Schema::hasTable('api_credentials') && DB::table('api_credentials')->count()) {
                // For Google Key
            ...
            if (Schema::hasTable('admin') && DB::table('admin')->count()) {
                $admin_email = @Admin::find(1)->email;
                View::share('admin_email', $admin_email);
            }

            if (Schema::hasTable('payment_gateway') && DB::table('payment_gateway')->count()) {
            ...
            if (Schema::hasTable('email_settings') && DB::table('email_settings')->count()) {
            ...
            if (Schema::hasTable('language') && DB::table('language')->count()) {
