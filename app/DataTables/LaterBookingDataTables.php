...
class LaterBookingDataTables extends DataTable
{
            ...
            ->editColumn('status', function ($schedule_rides) {
                if ($schedule_rides->status == 'Pending' && $schedule_rides->booking_type == 'Manual Booking') {
                    ...
                    if ($schedule_rides->schedule_cancel == null) {
                        ...
                        $status = 'Cancelled by ' . $schedule_rides->schedule_cancel->cancel_by .
                            '<br><span data-toggle="modal" data-target="#cancel_reason_popup" class="btn btn-primary 
                        cancel_button" schedule_id="' . $schedule_rides->id .
                            '" cancel_by="' . $schedule_rides->schedule_cancel->cancel_by .
                            '" cancel_reason="' . $schedule_rides->schedule_cancel->cancel_reason .
                            '">Cancel Reason</a>';
                    }
                } elseif ($schedule_rides->status == 'Car Not Found') {
                    $status = '<a href="pick-this-request/'.$schedule_rides->id.'">Assign to driver</a>'
                        // .'<a href="'.url().'">Assign to driver</a>'
                    ;
                } elseif (
                    $schedule_rides->status == 'Completed' && $schedule_rides->booking_type == 'Manual Booking'
                    && $schedule_rides->request->trips != null && $schedule_rides->request->trips->status == 'Cancelled'
                ) {
    ...
    {
        $schedule_rides =  $result = ScheduleRide::join('users as rider', function ($join) {
            ...
            ->where(function ($query) {
                ...
                if (LOGIN_USER_TYPE == 'company') {
                    $query->where('schedule_ride.company_id', Auth::guard('company')->user()->id)
                        ->orWhere('schedule_ride.company_id', Null)
